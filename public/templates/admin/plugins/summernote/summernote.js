...
var dropdown = renderer.create('<ul class="note-dropdown-product dropdown-product"></ul>', function ($node, options) {
...
var dropdownCheck = renderer.create('<ul class="note-dropdown-product dropdown-product note-check"></ul>', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    var value = typeof item === 'string' ? item : item.value || '';
    var content = options.template ? options.template(item) : item;
